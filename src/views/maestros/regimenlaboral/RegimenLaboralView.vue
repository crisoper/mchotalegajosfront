<template>
  <div v-loading="loadingData">
    ...
    >
      ...
      <el-table-column :align="'center'" width="120px" prop="activo_es" label="ACTIVO"/>
          ...
          <el-dropdown trigger="click" @command="handleActionsOpciones">
            ...
            <template #dropdown>
              <el-dropdown-menu ref="dropdown">
                <el-dropdown-item
                  :command="{ action: 'ADD' }"
                  :disabled="isActionDisabled('maestros.regimenlaboral.crear')" 
                >
                ...
                <!-- <el-dropdown-item
                  ...
                  :disabled="isActionDisabled('maestros.regimenlaboral.exportar')"
                > 
                  <span>
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item
                    ...
                    :disabled="isActionDisabled('maestros.regimenlaboral.actualizar')"
                  >
                    <span>
                  ...
                  <el-dropdown-item
                    ...
                    :disabled="isActionDisabled('maestros.regimenlaboral.eliminar')"
                  >
                    <span>
...
import RegimenLaboralForm from './components/RegimenLaboralForm.vue'
import { calcularAnchoDialog } from '@/utils/responsive'
import { useAuthStore } from "@/stores/auth";
import { nextTick } from 'vue'
const dataResource = new Resource('administracion/regimenlaboral')
...
export default {
  ...
  methods: {
    ...
    addItem() {
      ...
      this.titleForm = 'Registrar regimen laboral'
    ...
    editItem(item) {
      // console.log(item)
      this.titleForm = 'Modificar regimen laboral'
      this.openDialogCreate = true;
      this.$nextTick(() => {
    ...
    handleRowActions(commandData) {
      ...
      if (action === 'EDIT' && validPermision('maestros.regimenlaboral.actualizar')) {
        this.editItem(item)
      } else if (action === 'DELETE' && validPermision('maestros.regimenlaboral.eliminar')) {
    ...
    handleActionsOpciones(command) {
      ...
      if (action === 'ADD' && validPermision('maestros.regimenlaboral.crear')) {
        this.addItem()
      } else if (action === 'EXPORT' && validPermision('maestros.regimenlaboral.exportar')) {
        this.exportarDatos()
      } else {
        ElNotification({message: 'Usted no tiene permiso para realizar esta acción', type: 'info' })
      }
